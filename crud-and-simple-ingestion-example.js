...
async function writeRecords(items) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (r !== undefined) {
            const dimensions = [
                { 'Name': 'TransporterID', 'Value': r.TransporterID.toString() },
                { 'Name': 'ServiceArea', 'Value': r.ServiceArea.toString() },
                { 'Name': 'Email', 'Value': r.Email.toString() },
                { 'Name': 'APIName', 'Value': r.APIName.toString() }
            ];
            // console.log(dimensions)
            const GetOffersForProvider = {
                'Dimensions': dimensions,
                'MeasureName': 'NumberOffersReturned',
                'MeasureValue': '50',
                'MeasureValueType': 'INT',
                'Time': currentTime.toString()
            };

            records.push(GetOffersForProvider)
        }
    };
...
module.exports = {
    createDatabase, describeDatabase, updateDatabase, listDatabases, createTable, describeTable,
